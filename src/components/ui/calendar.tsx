...
function Calendar({
	...
	return (
		<DayPicker
			...
			className={cn(
				"group/calendar bg-white p-3 [--cell-size:--spacing(8)] dark:bg-zinc-950 [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent",
			...
			classNames={{
				...
				dropdown_root: cn(
					"relative has-focus:border-zinc-950 border border-zinc-200 shadow-xs has-focus:ring-zinc-950/50 has-focus:ring-[3px] rounded-md dark:has-focus:border-zinc-300 dark:border-zinc-800 dark:has-focus:ring-zinc-300/50",
				...
				caption_label: cn(
					...
					captionLayout === "label"
						? "text-sm"
						: "rounded-md pl-2 pr-1 flex items-center gap-1 text-sm h-8 [&>svg]:text-zinc-500 [&>svg]:size-3.5 dark:[&>svg]:text-zinc-400",
					defaultClassNames.caption_label
				),
				table: "w-full border-collapse",
				weekdays: cn("flex", defaultClassNames.weekdays),
				weekday: cn(
					"text-zinc-500 rounded-md flex-1 font-normal text-[0.8rem] select-none dark:text-zinc-400",
